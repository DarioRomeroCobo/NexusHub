...
describe('Integracion API YouTube: estadisticas', () => {
    let app;
    const correosCreados = new Set();

    jest.setTimeout(30000);

    const generarCorreoUnico = () => `it_api_stats_${Date.now()}_${Math.random().toString(36).slice(2, 8)}@nexushub.test`;
    ...
    test('GET /usuario/api/youtube/estadisticas devuelve estadisticas en JSON', async () => {
        ...
        expect(response.body.redes.length).toBe(2);
